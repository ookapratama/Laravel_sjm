          ...
          <div class="card-body">
            ...
            <div id="step1" class="step-content">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="username" class="form-control @error('username') is-invalid @enderror" required autocomplete="username" value="{{ old('username') }}">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" required autocomplete="name" value="{{ old('name', $pre->name) }}">
                @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
              </div>

              <div class="mb-3">
                <label class="form-label">Email</label>
                <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $pre->email) }}" readonly>
                @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
              </div>

              <div class="mb-3">
                <label class="form-label label-required">No. HP / WA</label>
                <input type="text" name="phone" class="form-control @error('phone') is-invalid @enderror" required autocomplete="tel" value="{{ old('phone', $pre->phone) }}">
            ...
            <div id="step2" class="step-content d-none">
              ...
              <div class="mb-3">
                ...
                @foreach(['islam' => 'Islam', 'kristen' => 'Kristen', 'katolik' => 'Katolik', 'budha' => 'Budha', 'hindu' => 'Hindu', 'lainnya' => 'Lainnya'] as $key => $label)
                  <div class="form-check form-check-inline">
                    <input class="form-check-input @error('agama') is-invalid @enderror" type="radio" name="agama" value="{{ $key }}" {{ old('agama')===$key ? 'checked' : '' }} required>
...
<datalist id="bankList"></datalist>
       ...
       placeholder="Pilih / ketik nama bank" required>
<datalist id="bankList"></datalist>
              ...
              <h5 class="fw-bold text-primary mt-4">Ahli Waris</h5>
              ...
              <div class="mb-3">
                <label class="form-label label-required">Hubungan</label>
              ...
              <div class="d-flex justify-content-between mt-4">
                ...
                <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> Simpan & Lanjut</button>
              </div>
            </div>
          </div>
        </div>
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('#step1 [required], #step2 [required]').forEach(el => el.setAttribute('data-required','1'));
    toggleRequired(step1, true);
    toggleRequired(step2, false);
  });
  ...
  document.getElementById('formWizard').addEventListener('submit', async function(e) {
    ...
    try {
      ...
      toastr.error('Terjadi kesalahan jaringan.'.data);
    } catch (err) {
      console.error(err);
      toastr.error('Terjadi kesalahan jaringan.');
    }
  });
    const bankInput = document.querySelector('input[name="nama_bank"]');
  if (bankInput) {
    bankInput.setAttribute('list', 'bankList');
    bankInput.setAttribute('placeholder', 'Pilih / ketik nama bank');
    const datalist = document.getElementById('bankList');

    // Urutan: BUMN dulu, lalu bank umum nasional, bank digital & syariah, terakhir BPD
    const BUMN = [
      'Bank Mandiri',
